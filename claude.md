...
- 2026-09-25 v0.2.2 を Releases に掲載（配信者も含めた均等ローテーションをガイドに追記、「列に並ぶ」の説明改善、初回起動時の配信者の列入り修正）
  - 視聴者から「自分も含めた均等ローテーションはできないのか」と質問 → ローテーション＋配信者「列に並ぶ」で可能。ガイドに書いていなかったのが原因
